...
static int max77854_fg_read_qh_vfsoc(struct max77854_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	int soc;
...
static int max77854_fg_read_qh(struct max77854_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	int ret;

	if (max77854_bulk_read(fuelgauge->i2c, QH_REG, 2, data) < 0) {
...
static int max77854_fg_read_soc(struct max77854_fuelgauge_data *fuelgauge)
{
	...
	if (!(fuelgauge->info.pr_cnt % PRINT_COUNT)) {
		...
		pr_debug("%s: REPSOC (%d), VFSOC (%d), data(0x%04x)\n",
				__func__, soc/10, max77854_fg_read_vfsoc(fuelgauge)/10, (data[1]<<8) | data[0]);
	}
#endif
...
int max77854_fg_reset_soc(struct max77854_fuelgauge_data *fuelgauge, int rec_ocv)
{
	u8 data[2], raw_vfocv[2] = {0,};
	int vfocv, fullcap;

	wake_lock(&fuelgauge->fg_reset_wake_lock);

	/* delay for current stablization */
	msleep(500);

	if (rec_ocv) {
		max77854_write_word(fuelgauge->i2c, COFFSET_REG, 0x1);
		max77854_bulk_read(fuelgauge->i2c, MISCCFG_REG, 2, data);
		data[0] |= (0x1 << 2);
		max77854_bulk_write(fuelgauge->i2c, MISCCFG_REG, 2, data);
		max77854_bulk_read(fuelgauge->i2c, VFOCV_REG, 2, raw_vfocv);
		max77854_bulk_write(fuelgauge->i2c, VCELL_REG, 2, raw_vfocv);
	}
	...
	if (max77854_bulk_read(fuelgauge->i2c, MISCCFG_REG,
		...
		wake_unlock(&fuelgauge->fg_reset_wake_lock);
		return -1;
	}
	...
	msleep(250);
	if (rec_ocv) {
		pr_info("%s: Recover dpacc/dqacc\n", __func__);
		max77854_bulk_read(fuelgauge->i2c, MISCCFG_REG, 2, data);
		data[0] &= ~(0x1 << 2);
		max77854_bulk_write(fuelgauge->i2c, MISCCFG_REG, 2, data);
		max77854_write_word(fuelgauge->i2c, COFFSET_REG, 0x1);
		if (fuelgauge->battery_data->dpacc > 0 && fuelgauge->battery_data->dqacc > 0) {
			max77854_write_word(fuelgauge->i2c, DPACC_REG, fuelgauge->battery_data->dpacc);
			max77854_write_word(fuelgauge->i2c, DQACC_REG, fuelgauge->battery_data->dqacc);
		}
		fuelgauge->bad_coffset = false;
	}
...
int max77854_get_fuelgauge_value(struct max77854_fuelgauge_data *fuelgauge, int data)
{
	...
	case FG_QH:
		ret = max77854_fg_read_qh(fuelgauge);
		break;

	case FG_QH_VF_SOC:
		ret = max77854_fg_read_qh_vfsoc(fuelgauge);
		break;
	default:
		ret = -1;
		break;
...
bool max77854_fg_reset(struct max77854_fuelgauge_data *fuelgauge, int rec_ocv)
{
	if (!max77854_fg_reset_soc(fuelgauge, rec_ocv))
		return true;
	else
		return false;
}
...
static void max77854_check_coffset(struct max77854_fuelgauge_data *fuelgauge)
{
	u16 data, vfsoc, soc;

	data = max77854_read_word(fuelgauge->i2c, COFFSET_REG);
	pr_info("%s: FG COFFSET : 0x%x\n", __func__, data);
	if (data > 1) {
		max77854_write_word(fuelgauge->i2c, COFFSET_REG, 0x1);
		vfsoc = max77854_fg_read_vfsoc(fuelgauge);
		soc = max77854_fg_read_soc(fuelgauge);

		if (!fuelgauge->is_charging) {
			fuelgauge->bad_coffset = true;
		} else if (vfsoc >= 1000 && soc >= 1000) {
                        max77854_fg_reset(fuelgauge, 1);
		}
	}
}
...
static bool make_fuelgauge_sbm_data(struct max77854_fuelgauge_data *fuelgauge,
			union power_supply_propval *val)
{
	...
	switch(fuelgauge->pdata->sbm_data_type) {
		...
		case SBM_DATA_FULL_2ND:
			sprintf(str, ",cap_rep:%d,rep_cap:%d", 
				max77854_fg_read_fullcaprep(fuelgauge),
				max77854_fg_read_repcap(fuelgauge));
			break;
		case SBM_DATA_FG_VEMPTY:
			sprintf(str, ",vcell:%d,ocv:%d", 
				max77854_fg_read_vcell(fuelgauge),
				max77854_fg_read_vfocv(fuelgauge));
			break;
			     ...
			     union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		/* check coffset value */
		max77854_check_coffset(fuelgauge);
			     ...
			     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		if (val->intval == SEC_FUELGAUGE_CAPACITY_TYPE_RESET) {
			fuelgauge->initial_update_of_soc = true;
			if (!max77854_fg_reset(fuelgauge, 0))
				return -EINVAL;
			else
				break;
		}
	case POWER_SUPPLY_PROP_TEMP:
		...
		if (val->intval < 0 && !low_temp_wa) {
			low_temp_wa = true;
			max77854_write_word(fuelgauge->i2c, 0x29, 0xCEA7);
			pr_info("%s : FilterCFG(0x%0x)\n", __func__, max77854_read_word(fuelgauge->i2c, 0x29));
		} else if (val->intval > 30 && low_temp_wa) {
			low_temp_wa = false;
			max77854_write_word(fuelgauge->i2c, 0x29, 0xCEA4);
			pr_info("%s : FilterCFG(0x%0x)\n", __func__, max77854_read_word(fuelgauge->i2c, 0x29));
		}
...
static int max77854_fuelgauge_resume(struct device *dev)
{
	...
	fuelgauge->initial_update_of_soc = true;
	if (fuelgauge->bad_coffset && !fuelgauge->is_charging) {
		max77854_fg_reset(fuelgauge, 1);
	}
